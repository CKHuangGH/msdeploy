...
for j in `seq 1 $clustercount`
		do
			ssh root@$i kind create cluster --image kindest/node:v1.18.0 --config /root/mck8s_lsv/remote_script/config_03/kind-example-config-$j.yaml --name cluster$clustername
